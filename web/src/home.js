...
function card(p){return `<article class="playerCardItem ${String(p.rarity).toLowerCase()}"><span class="rarity">${esc(p.rarity)}</span><strong>${esc(p.name)}</strong><span>${esc(p.position)} • ${p.overall}</span><small>${esc(p.nation||"World")}</small></article>`}
function panel(kind){
 ...
 if(kind==="squad"){const ps=PLAYER_POOL.slice(0,11);$("#panelBody").innerHTML=`<div class="formation"><div class="pitchMini">${ps.map((p,i)=>`<div class="miniPlayer" style="--i:${i}">${esc(p.name.split(" ").pop())}</div>`).join("")}</div><div class="formationInfo"><span>4-3-3</span><b>WORLD XI</b><small>Possession • Balanced</small></div></div><div class="sectionTitle">STARTING XI <span>11 / 11</span></div><div class="playerList">${ps.map((p,i)=>`<div class="listRow"><b>${String(i+1).padStart(2,"0")}</b><span>${esc(p.name)}</span><small>${p.position} • ${p.overall}</small></div>`).join("")}</div><button class="primary wide" id="squadPlay">PLAY WITH THIS XI</button>`;$("#squadPlay").onclick=start;return}
...
document.querySelectorAll("[data-nav]").forEach(b=>b.onclick=()=>b.dataset.nav==="home"?home():panel(b.dataset.nav));wallet();